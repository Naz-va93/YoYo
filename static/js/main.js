		...
		if(list) {
			...
			target.addEventListener('click', function () {
				if(!target.classList.contains('_animating')) {
					...
					let heightDropDownList = list.offsetHeight ? list.offsetHeight : 0, 
...
body.addEventListener('click', function (event) {
	...
	if(favoriteBtn) {
		if (!favoriteBtn.classList.contains('ignore-toggle')){
...
const slimSelect = document.querySelectorAll('.custom-select');
...
const addCoinAvatar = document.querySelector('.add-coin__avatar input');
function getImgData(files, block) {
	const file = files[0];
	if (file) {
	  const fileReader = new FileReader();
	  fileReader.readAsDataURL(file);
	  fileReader.addEventListener("load", function () {
		block.insertAdjacentHTML("beforeend", '<img src="' + this.result + '" />');
	  });    
	}
}
if(addCoinAvatar) {
	addCoinAvatar.addEventListener('change', function (event) {
		if(addCoinAvatar.closest('.add-coin__avatar').querySelector('img')) addCoinAvatar.closest('.add-coin__avatar').querySelector('img').remove();
		getImgData(addCoinAvatar.files, addCoinAvatar.closest('.add-coin__avatar'));
	})
}

const promoteBlockBanner = document.querySelectorAll('.promote__block--banner input');
promoteBlockBanner.forEach(promoteBlockBanner => {
	promoteBlockBanner.addEventListener('change', function (event) {
		if(promoteBlockBanner.closest('label').querySelector('img')) promoteBlockBanner.closest('label').querySelector('img').remove();
		getImgData(promoteBlockBanner.files, promoteBlockBanner.closest('label'));
	})
})
